...
class QuantAnalysisOutput(BaseModel):
    ...
    financial_health_score: int = Field(
        description="Score 1-10 based on Debt/Equity, Current Ratio"
    )
    key_metrics: Dict[str, Any] = Field(
        description="Selected key metrics extracted from data"
    )

    # New Fields for Advanced Reasoning
    smart_money_score: int = Field(
        description="Score 1-10 based on Insider/Institutional activity"
    )
    short_squeeze_risk: Literal["High", "Medium", "Low", "None"] = Field(
        description="Assessment of short squeeze potential"
    )

    summary: str = Field(
        description="Quantitative summary of the company's financial position"
    )
...
4. `get_insider_trades`: Analyze "Smart Money" flow (Cluster Buying vs Dumping).
5. `get_ownership_data`: Check Institutional Sponsorship & Short Squeeze Risk.

**ANALYSIS PROCESS (Chain of Thought):**
1. **Data Collection**: Gather all the raw numbers first.
2. **Smart Money Check**:
   - Are insiders buying? (Strongest Signal).
   - Is institutional ownership high (>70%)? (Quality).
   - Is Short Float > 15%? (Squeeze Potential or Fraud?).
3. **Growth Analysis**:
...
6. **Conclusion**: Assign a pure mathematical score (0-10) and flag specific "Smart Money" signals.
